...
PreparedStatement.prototype.setBinaryStream = function(index, val, length, callback) {
  ...
  if (val != null) {
    var cname = val.constructor.name;
    if (!/^nodeJava_java_io_\S*InputStream$/.test(cname)) {
      if (cname !== 'nodeJava_aB') {
        if (!(val instanceof Int8Array) &&
            !(val instanceof Uint8Array) &&
            !(val instanceof Uint8ClampedArray))
          return callback(new Error("Invalid value type: expected Int8Array or Uint8Array or Uint8ClampedArray, but was "+ cname));
        val = java.newArray('byte', Array.prototype.slice.call(val));
      }
      val = java.newInstanceSync("java.io.ByteArrayInputStream", val);
    }
  }

  if (length == null) {
    this._ps.setBinaryStream(index, val, function(err) {
      if (err) return callback(err);
      callback(null);
    });
  } else {
    this._ps.setBinaryStream(index, val, length, function(err) {
      if (err) return callback(err);
      callback(null);
    });
  }
...
PreparedStatement.prototype.setCharacterStream = function(index, val, length, callback) {
  ...
  if (val != null) {
    var cname = val.constructor.name;
    if (!/^nodeJava_java_io_\S*Reader$/.test(cname)) {
      if (typeof val !== 'string')
        val = val.toString();
      val = java.newInstanceSync("java.io.StringReader", val);
    }
  }

  if (length == null) {
    this._ps.setCharacterStream(index, val, function(err) {
      if (err) return callback(err);
      callback(null);
    });
  } else {
    this._ps.setCharacterStream(index, val, length, function(err) {
      if (err) return callback(err);
      callback(null);
    });
  }
...
PreparedStatement.prototype.setDate = function(index, val, calendar, callback) {
  ...
  if (val != null) {
    var cname = val.constructor.name;
    if (cname !== 'nodeJava_java_sql_Date') {
      if (!(val instanceof Date)) {
        if (typeof val !== 'string' && typeof val !== 'number')
          val = val.toString();
        val = new Date(val);
      }
      var ms = java.newLong(val.getTime());
      val = java.newInstanceSync("java.sql.Date", ms);
    }
  }
...
PreparedStatement.prototype.setLong = function(index, val, callback) {
  if (val != null) {
    var cname = val.constructor.name;
    if (cname !== 'nodeJava_java_lang_Long') {
      if (typeof val === 'number') {
        val = java.newLong(val);
      } else {
        val = java.callStaticMethodSync('java.lang.Long', 'parseLong', val.toString());
      }
    }
  }
...
PreparedStatement.prototype.setShort = function(index, val, callback) {
  if (val != null) {
    var cname = val.constructor.name;
    if (cname !== 'nodeJava_java_lang_Short') {
      if (typeof val === 'number') {
        val = java.newShort(val);
      } else {
        val = java.callStaticMethodSync('java.lang.Short', 'parseShort', val.toString());
      }
    }
  }

  this._ps.setShort(index, val, function(err) {
    if (err) return callback(err);
    callback(null);
  });
};
...
PreparedStatement.prototype.setTime = function(index, val, calendar, callback) {
  ...
  if (val != null) {
    var cname = val.constructor.name;
    if (cname !== 'nodeJava_java_sql_Time') {
      if (!(val instanceof Date)) {
        if (typeof val !== 'string' && typeof val !== 'number')
          val = val.toString();
        val = new Date(val);
      ...
      var ms = java.newLong(val.getTime());
      val = java.newInstanceSync("java.sql.Time", ms);
    }
  }
...
PreparedStatement.prototype.setTimestamp = function(index, val, calendar, callback) {
  ...
  if (val != null) {
    var cname = val.constructor.name;
    if (cname !== 'nodeJava_java_sql_Timestamp') {
      if (!(val instanceof Date)) {
        if (typeof val !== 'string' && typeof val !== 'number')
          val = val.toString();
        val = new Date(val);
      ...
      var ms = java.newLong(val.getTime());
      val = java.newInstanceSync("java.sql.Timestamp", ms);
    }
  }
...
PreparedStatement.prototype.setObject = function(index, val, type, scalelen, callback) {
  var args = Array.prototype.slice.call(arguments);
  callback = args.pop();
  index    = args[0];
  val      = args[1];
  ...
  if (type == null) {
    this._ps.setObject(index, val, function(err) {
      if (err) return callback(err);
      callback(null);
    });
  }
  else if (scalelen == null) {
    this._ps.setObject(index, val, type, function(err) {
      if (err) return callback(err);
      callback(null);
    });
  }
  else {
    this._ps.setObject(index, val, type, scalelen, function(err) {
      if (err) return callback(err);
      callback(null);
    });
  }
